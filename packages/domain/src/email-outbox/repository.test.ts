...
describe("EmailOutboxRepository", () => {
  ...
  layer(TestLayer)("repository", (it) => {
    ...
    it.effect("rejects a corrupt persisted intent payload with a typed error", () =>
      Effect.gen(function* () {
        const organizationId = yield* WorkspaceId.generate;
        const repository = yield* EmailOutboxRepository;
        const db = yield* currentDb;
    ...
    it.effect("coalesces pending status changes into their final payload only", () =>
      Effect.gen(function* () {
        const organizationId = yield* WorkspaceId.generate;
        const repository = yield* EmailOutboxRepository;
        const db = yield* currentDb;
    ...
    it.effect("keeps a product mutation and recorded intent in one transaction", () =>
      Effect.gen(function* () {
        const repository = yield* EmailOutboxRepository;
        const organizationId = yield* WorkspaceId.generate;
        ...
        yield* Effect.flip(
          transaction(
            Effect.gen(function* () {
              yield* createOrganization(organizationId);
              yield* repository.recordIntent(submissionIntent(organizationId));
              return yield* Effect.fail("abort email outbox test transaction");
            })
          )
        );
