...
describe("email entitlement policy", () => {
  layer(TestLayer)("plan-aware email decisions", (it) => {
    it.effect("allows one free submission recipient but no subscriber email", () =>
      Effect.gen(function* () {
    ...
    it.effect("allows paid subscriber email and unlimited submission recipients", () =>
      Effect.gen(function* () {
        const db = yield* currentDb;
        const policy = yield* EntitlementPolicy;
        const organizationId = yield* WorkspaceId.generate;
