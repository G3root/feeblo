...
describe("NotificationRpcHandlers", () => {
  ...
  const TestLayer = Layer.mergeAll(NotificationService.layer, NotificationPolicy.layer).pipe(
  ...
  layer(Layer.merge(TestLayer, Database.PgliteDatabaseLive))("handlers", (it) => {
    it.effect("lists and marks only the current member's notifications", () =>
      Effect.gen(function* () {
        ...
        const scoped = <A, E, R>(effect: Effect.Effect<A, E, R>) => effect.pipe(Effect.provideService(CurrentSession, session(fixture)));

        expect(yield* scoped(handlers.NotificationUnreadCount({ organizationId: fixture.organizationId }))).toEqual({ count: 1 });
        expect(yield* scoped(handlers.NotificationList({ organizationId: fixture.organizationId }))).toMatchObject([{ id, readAt: null }]);
        yield* scoped(handlers.NotificationMarkRead({ organizationId: fixture.organizationId, notificationId: id }));
        expect(yield* scoped(handlers.NotificationUnreadCount({ organizationId: fixture.organizationId }))).toEqual({ count: 0 });
      })
    );

    it.effect("denies notification access outside the current membership", () =>
      Effect.gen(function* () {
        const handlers = yield* NotificationRpcHandlersEffect;
        const fixture = yield* makeFixture();
        const error = yield* Effect.flip(
          handlers.NotificationList({ organizationId: fixture.organizationId }).pipe(
    ...
    it.effect("marks every unread notification read without changing another member's inbox", () =>
      Effect.gen(function* () {
        const handlers = yield* NotificationRpcHandlersEffect;
        const fixture = yield* makeFixture();
        ...
        yield* insertNotification(fixture);
        yield* insertNotification(fixture, { recipientMemberId: otherMember.memberId });
        const scoped = <A, E, R>(effect: Effect.Effect<A, E, R>) => effect.pipe(Effect.provideService(CurrentSession, session(fixture)));
        const otherScoped = <A, E, R>(effect: Effect.Effect<A, E, R>) => effect.pipe(Effect.provideService(CurrentSession, session(otherMember)));

        yield* scoped(handlers.NotificationMarkAllRead({ organizationId: fixture.organizationId }));
        expect(yield* scoped(handlers.NotificationUnreadCount({ organizationId: fixture.organizationId }))).toEqual({ count: 0 });
        expect(yield* otherScoped(handlers.NotificationUnreadCount({ organizationId: fixture.organizationId }))).toEqual({ count: 1 });
      })
    );

    it.effect("paginates newest-first and never exposes another member's notification", () =>
      Effect.gen(function* () {
        const handlers = yield* NotificationRpcHandlersEffect;
        const fixture = yield* makeFixture();
        const otherMember = yield* addMember(fixture);
        const oldest = yield* insertNotification(fixture, { createdAt: new Date("2026-01-01T00:00:00.000Z") });
        const newest = yield* insertNotification(fixture, { createdAt: new Date("2026-01-01T00:02:00.000Z") });
        yield* insertNotification(fixture, { recipientMemberId: otherMember.memberId, createdAt: new Date("2026-01-01T00:03:00.000Z") });
        const scoped = <A, E, R>(effect: Effect.Effect<A, E, R>) => effect.pipe(Effect.provideService(CurrentSession, session(fixture)));

        const firstPage = yield* scoped(handlers.NotificationList({ organizationId: fixture.organizationId, limit: 1 }));
        expect(firstPage).toMatchObject([{ id: newest }]);
        const secondPage = yield* scoped(handlers.NotificationList({ organizationId: fixture.organizationId, cursor: firstPage[0]!.createdAt, limit: 1 }));
    ...
    it.effect("does not mark another member's notification as read", () =>
      Effect.gen(function* () {
        ...
        const fixture = yield* makeFixture();
        const otherMember = yield* addMember(fixture);
        const otherId = yield* insertNotification(fixture, { recipientMemberId: otherMember.memberId });
        const scoped = <A, E, R>(effect: Effect.Effect<A, E, R>) => effect.pipe(Effect.provideService(CurrentSession, session(fixture)));
        const otherScoped = <A, E, R>(effect: Effect.Effect<A, E, R>) => effect.pipe(Effect.provideService(CurrentSession, session(otherMember)));

        yield* scoped(handlers.NotificationMarkRead({ organizationId: fixture.organizationId, notificationId: otherId }));
        expect(yield* otherScoped(handlers.NotificationUnreadCount({ organizationId: fixture.organizationId }))).toEqual({ count: 1 });
      })
    );
  });
