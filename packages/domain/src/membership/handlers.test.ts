...
describe("MembershipRpcHandlers", () => {
  ...
  const makeSession = (fixture: Fixture, isMember = true): Session => ({
    ...
    session: { userId: fixture.userId, token: "test-token" }, organizations: [{ id: fixture.organizationId }],
    memberships: isMember ? [{ membershipId: fixture.membershipId, organizationId: fixture.organizationId, role: "owner" }] : [],
  });
  const makeFixture = () => Effect.gen(function* () {
    const db = yield* currentDb; const organizationId = yield* WorkspaceId.generate; const userId = `user_${organizationId}`; const membershipId = `membership_${organizationId}`; const now = new Date();
  ...
  const AuthTest = Layer.succeed(Auth, { handler: () => new Response(), api: { getSession: async () => null, createInvitation: async () => undefined } });
  ...
  layer(TestLayer)("handlers", (it) => {
    it.effect("lists the current user's memberships", () => Effect.gen(function* () {
      const handlers = yield* MembershipRpcHandlersEffect; const fixture = yield* makeFixture();
      const memberships = yield* handlers.MembershipList().pipe(Effect.provideService(CurrentSession, makeSession(fixture)));
      expect(memberships).toMatchObject([{ id: fixture.membershipId, organizationId: fixture.organizationId }]);
    }));
    it.effect("rejects non-members from listing organization members", () => Effect.gen(function* () {
      const handlers = yield* MembershipRpcHandlersEffect; const fixture = yield* makeFixture();
      const error = yield* Effect.flip(handlers.OrganizationMembersList({ organizationId: fixture.organizationId }).pipe(Effect.provideService(CurrentSession, makeSession(fixture, false))));
