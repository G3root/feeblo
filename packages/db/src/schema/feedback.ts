...
export const notificationBatchTable = pgTable(
  "notification_batch",
  {
    id: text("id").primaryKey(),
    kind: notificationBatchKindEnum("kind").notNull(),
    ...
    initialStatusId: text("initial_status_id"),
    latestStatusId: text("latest_status_id"),
    latestActorUserId: text("latest_actor_user_id").references(
      () => userTable.id,
      { onDelete: "set null" }
    ),
    ...
    status: notificationBatchStatusEnum("status").default("QUEUED").notNull(),
  ...
  (table) => [
    index("notification_batch_recovery_idx").on(table.status, table.deadline),
    index("notification_batch_post_idx").on(table.postId),
    uniqueIndex("notification_batch_active_resource_uidx")
      .on(table.kind, table.resourceId)
      .where(sql`${table.status} in ('QUEUED', 'DISPATCHED', 'PROCESSING')`),
    uniqueIndex("notification_batch_once_resource_uidx")
      .on(table.kind, table.resourceId)
      .where(sql`${table.kind} = 'WEEKLY_DIGEST'`),
  ]
);

export const submissionQueueTable = pgTable(
  "submission_queue",
  {
    postId: text("post_id")
      .primaryKey()
  ...
  (table) => [
    index("submission_queue_organization_created_at_idx").on(
      table.organizationId,
      table.createdAt
    ),
  ]
...
export const notificationDeliveryTable = pgTable(
  "notification_delivery",
  {
    ...
    batchId: text("batch_id")
      .notNull()
      .references(() => notificationBatchTable.id, { onDelete: "cascade" }),
    userId: text("user_id")
      .notNull()
      .references(() => userTable.id, { onDelete: "cascade" }),
  ...
  (table) => [
    uniqueIndex("notification_delivery_batch_user_uidx").on(
      table.batchId,
      table.userId
    ),
    uniqueIndex("notification_delivery_message_id_uidx").on(table.messageId),
