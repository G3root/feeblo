...
>(table: TTable, refine?: TRefine): BuildInsertSchema<TTable, TRefine> {
    ...
    const refinedEntries = Object.entries(refine).map(
      ([name, refineColumn]) => [
        ...
        typeof refineColumn === "function" && !Schema.isSchema(refineColumn)
          ? (refineColumn as (schema: Schema.Top) => Schema.Top)(
              schemaEntries[name]!
            )
          : refineColumn,
      ]
  ...
  for (const [name, column] of columnEntries) {
    if (!column.notNull) {
      schemaEntries[name] = Schema.optional(
        Schema.NullOr(schemaEntries[name] as Schema.Top)
      );
  ...
  return Schema.Struct(schemaEntries) as unknown as BuildInsertSchema<
    TTable,
    TRefine
  >;
}
...
>(table: TTable, refine?: TRefine): BuildSelectSchema<TTable, TRefine> {
  ...
  return Schema.Struct(schemaEntries) as unknown as BuildSelectSchema<
    TTable,
    TRefine
  >;
}
...
function mapColumnToSchema(column: Drizzle.Column): Schema.Schema<any> {
  ...
  if (isWithEnum(column)) {
    type =
      column.enumValues.length > 0
        ? Schema.Literals(column.enumValues)
        : Schema.String;
  }
  ...
  if (!type) {
    if (Drizzle.is(column, DrizzlePg.PgUUID)) {
      ...
      type =
        hasMode(column) && column.mode === "number"
          ? Schema.Number
          : Schema.BigInt;
