...
describe("makeLegid", () => {
  const factory = makeLegid("post", { approximateLength: 12 });

  describe("generate", () => {
    it("generates a non-empty alphanumeric id", async () => {
    ...
    it("unsafeGenerate returns a Promise", async () => {
  ...
  describe("verify", () => {
    ...
    it("rejects a random string", async () => {
      const isValid = await Effect.runPromise(factory.verify("admin"));
  ...
  describe("parse", () => {
    ...
    it("fails with LegidError on an invalid id", async () => {
      const result = await run(factory.parse("admin"));
    ...
    it("fails with LegidError on malformed input", async () => {
      const result = await run(factory.parse(""));

      expect(result).toBeInstanceOf(LegidError);
    });

    it("fails with LegidError on input exceeding max length", async () => {
      const result = await run(factory.parse("a".repeat(100)));

      expect(result).toBeInstanceOf(LegidError);
    });

    it("fails with LegidError on input with invalid characters", async () => {
      const result = await run(factory.parse("invalid_id!"));
  ...
  describe("is", () => {
    it("returns true for a well-formed id", async () => {
      const id = await Effect.runPromise(factory.generate);
    ...
    it("returns false for input with invalid characters", () => {
      expect(factory.is("invalid_id!")).toBe(false);
    });

    it("returns false for input exceeding max length", () => {
      expect(factory.is("a".repeat(100))).toBe(false);
    });
  });
  ...
  describe("brand", () => {
    ...
    it("uses a custom brand name when provided", () => {
      const custom = makeLegid("post", { brand: "MyPostId" });

      expect(custom.brand).toBe("MyPostId");
    });

    it("capitalizes snake_case names into PascalCase brand names", () => {
      const factory = makeLegid("post_reaction");

      expect(factory.brand).toBe("PostReactionId");
    });
  });

  describe("type-level brand", () => {
    it("the generated id is assignable to the branded type", async () => {
      const id = await Effect.runPromise(factory.generate);
      const branded: LegidOf<"PostId"> = id;
  ...
  describe("custom config", () => {
    it("honors a custom salt for verification", async () => {
      const salted = makeLegid("post", { salt: "custom-salt:" });
      const id = await Effect.runPromise(salted.generate);

      expect(await Effect.runPromise(salted.verify(id))).toBe(true);

      // The default-salt factory cannot verify ids generated with a custom salt
      expect(await Effect.runPromise(factory.verify(id))).toBe(false);
    });

    it("honors a custom approximate length", async () => {
      const long = makeLegid("post", { approximateLength: 20 });
      const short = makeLegid("post", { approximateLength: 8 });

      const longId = await Effect.runPromise(long.generate);
      const shortId = await Effect.runPromise(short.generate);
...
describe("makePrefixableLegid", () => {
  const factory = makePrefixableLegid("post", "pst", { approximateLength: 12 });
  ...
  describe("brand", () => {
    ...
    it("capitalizes snake_case names into PascalCase brand names", () => {
      const factory = makePrefixableLegid("comment_reaction", "crt");
  ...
  describe("prefix validation", () => {
    it("throws on a prefix with uppercase letters", () => {
      expect(() => makePrefixableLegid("post", "PST")).toThrow();
    });

    it("throws on a prefix with digits", () => {
      expect(() => makePrefixableLegid("post", "ps1")).toThrow();
    });

    it("throws on an empty prefix", () => {
      expect(() => makePrefixableLegid("post", "")).toThrow();
    });

    it("throws on a prefix with underscores", () => {
      expect(() => makePrefixableLegid("post", "p_s")).toThrow();
    });
  });
});
...
describe("namespacing across factories", () => {
  ...
  it("a prefixed id is not valid for a factory with a different prefix", async () => {
    const post = makePrefixableLegid("post", "pst", { approximateLength: 12 });
    const comment = makePrefixableLegid("comment", "cmt", {
  ...
  it("factories with different salts cannot verify each other's ids", async () => {
    const a = makeLegid("post", { salt: "salt-a:" });
    const b = makeLegid("post", { salt: "salt-b:" });

    const idA = await Effect.runPromise(a.generate);

    expect(await Effect.runPromise(a.verify(idA))).toBe(true);
    expect(await Effect.runPromise(b.verify(idA))).toBe(false);
  });
});
