...
> generating, or refactoring React and React codebases. Humans  
...
Comprehensive performance optimization guide for React and React applications, designed for AI agents and LLMs. Contains 40+ rules across 8 categories, prioritized by impact from critical (eliminating waterfalls, reducing bundle size) to incremental (advanced patterns). Each rule includes detailed explanations, real-world examples comparing incorrect vs. correct implementations, and specific impact metrics to guide automated refactoring and code generation.
...
Reference: [https://vercel.com/blog/how-we-optimized-package-imports-in-react](https://vercel.com/blog/how-we-optimized-package-imports-in-react)
...
React documentation explicitly states: "Treat Server Actions with the same security considerations as public-facing API endpoints, and verify if the user is allowed to perform a mutation."
...
Reference: [https://react.org/docs/app/guides/authentication](https://react.org/docs/app/guides/authentication)
...
In React, the `fetch` API is automatically extended with request memoization. Requests with the same URL and options are automatically deduplicated within a single request, so you don't need `React.cache()` for `fetch` calls. However, `React.cache()` is still essential for other async tasks:
...
Use React's `after()` to schedule work that should execute after a response is sent. This prevents logging, analytics, and other side effects from blocking the response.
...
Reference: [https://react.org/docs/app/api-reference/functions/after](https://react.org/docs/app/api-reference/functions/after)
...
In SSR frameworks (e.g., React), some values are intentionally different on server vs client (random IDs, dates, locale/timezone formatting). For these *expected* mismatches, wrap the dynamic text in an element with `suppressHydrationWarning` to prevent noisy warnings. Do not use this to hide real bugs. Don’t overuse it.
...
2. [https://react.org](https://react.org)
3. [https://swr.vercel.app](https://swr.vercel.app)
4. [https://github.com/shuding/better-all](https://github.com/shuding/better-all)
5. [https://github.com/isaacs/node-lru-cache](https://github.com/isaacs/node-lru-cache)
6. [https://vercel.com/blog/how-we-optimized-package-imports-in-react](https://vercel.com/blog/how-we-optimized-package-imports-in-react)
