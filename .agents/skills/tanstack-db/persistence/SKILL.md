...
This skill builds on tanstack-db and tanstack-db/collection-setup. Read those first.
...
See also: ../collection-setup/SKILL.md — for adapter selection and collection configuration.

See also: the `offline` skill in `@tanstack/offline-transactions` — for offline transaction queueing (complements persistence).
