...
This skill builds on the tanstack-db skill. Read it first for collection setup, query builder, and mutation patterns.
...
See ../live-queries/SKILL.md for full includes rules (correlation conditions, nested includes, aggregates).
...
The query builder looks like SQL but has constraints that SQL doesn't — equality joins only, orderBy required for limit/offset, no distinct without select. Agents write SQL-style queries that violate these constraints. See ../live-queries/SKILL.md § Common Mistakes for all constraints.

See also: ../live-queries/SKILL.md — for query builder API and all operators.

See also: ../mutations-optimistic/SKILL.md — for mutation patterns.
