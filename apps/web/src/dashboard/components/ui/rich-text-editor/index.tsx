...
const UPLOADED_IMAGE_PRELOAD_TIMEOUT_MS = 8000;
...
export const Editor = forwardRef<EditorHandle, EditorProps>(function Editor(
  ...
  const tiptapSurfaceClass = cn(
    "min-h-16 w-full bg-transparent px-3 py-2 text-base outline-none transition-[color,box-shadow] selection:bg-primary selection:text-primary-foreground placeholder:text-muted-foreground md:text-sm dark:bg-input/30 [&_img[data-upload-error]]:ring-2 [&_img[data-upload-error]]:ring-destructive [&_img[data-upload-error]]:ring-offset-2 [&_img[data-upload-error]]:ring-offset-background [&_img[data-uploading=true]]:animate-pulse [&_img[data-uploading=true]]:opacity-70 [&_p.is-empty::before]:pointer-events-none [&_p.is-empty::before]:float-left [&_p.is-empty::before]:h-0 [&_p.is-empty::before]:text-muted-foreground [&_p.is-empty::before]:content-[attr(data-placeholder)]",
    editorClassName
  );
  ...
  const editor = useEditor({
    ...
    editorProps: {
      ...
      handlePaste: (_view, event) => {
        if (!(enableImages && enableImagePasteDrop)) {
          return false;
        }
      ...
      handleDrop: (view, event, _slice, moved) => {
        if (moved || !enableImages || !enableImagePasteDrop) {
          return false;
        }
  ...
  const insertLocalImageFile = async (
    ...
    try {
      ...
      finalizeImageUpload(uploadId, (attrs): UploadableImageAttrs | null => {
        ...
        if (!expectedBlob || currentSrc !== expectedBlob) {
          return null;
        }
