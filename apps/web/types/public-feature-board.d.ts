 ...
 * interface. That package is typechecked by its own `check-types` task, so
 * pulling its sources into this app's program would register two different
 * routers in one program — a conflict with no correct resolution, because
 * each app's links are only valid against its own route tree.
 *
