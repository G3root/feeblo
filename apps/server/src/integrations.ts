...
const eventTypes = SUBSCRIBABLE_INTEGRATION_EVENT_TYPES;
...
): Effect.Effect<Option.Option<Date>, BadRequestError> => {
  if (cursor === undefined) {
    return Effect.succeed(Option.none());
  }
  return Schema.decodeUnknownEffect(Schema.DateFromString)(cursor).pipe(
...
/** Statically composed webhook kernel, management service, and scoped worker. */
export const makeIntegrationLayers = Effect.gen(function* () {
  ...
  const endpointSecurityPolicy = {
    allowPrivateNetworkInDevelopment: config.integrationAllowPrivateNetwork,
  ...
  const management = {
    listEndpoints: ({ organizationId }: { readonly organizationId: string }) =>
      Effect.gen(function* () {
        const rows = yield* db
          .select({
      ...
      }).pipe(mapManagementError("list")),
    createEndpoint: (input: {
      readonly endpointUrl: string;
      readonly eventTypes: readonly (typeof eventTypes)[number][];
      ...
      }).pipe(mapManagementError("create")),
    updateEndpoint: (input: {
      ...
      }).pipe(mapManagementError("update")),
    pauseEndpoint: ({
      connectionId,
      organizationId,
    }: {
    ...
    getDeliveryHistory: ({
      ...
      Effect.gen(function* () {
        const before = yield* decodeCursor(cursor);
        const rows = yield* db
          .select({
          ...
          .where(
            and(
              eq(schema.integrationDeliveryTable.connectionId, connectionId),
              eq(
                schema.integrationDeliveryTable.organizationId,
                organizationId
              ),
              ...(Option.isNone(before)
                ? []
                : [lt(schema.integrationDeliveryTable.createdAt, before.value)])
            )
          )
          .orderBy(desc(schema.integrationDeliveryTable.createdAt))
        ...
        return yield* Schema.decodeUnknownEffect(WebhookDeliveryHistoryPage)({
          ...
          nextCursor:
            rows.length > pageRows.length
              ? (pageRows.at(-1)?.delivery.createdAt.toISOString() ?? null)
              : null,
        }).pipe(
          Effect.mapError(
            () =>
      ...
      }).pipe(mapManagementError("history")),
    retryDelivery: ({
      deliveryId,
      organizationId,
    }: {
        ...
        .transaction(() =>
          Effect.gen(function* () {
            ...
            const [route] = yield* db
              ...
              .limit(1);
            if (connection?.lifecycle !== "active" || route?.enabled !== true) {
              return yield* new BadRequestError({
                message: "Resume the webhook endpoint before retrying delivery",
              });
            }
            const now = yield* DateTime.nowAsDate;
            yield* db
  ...
  return {
    layer: Layer.mergeAll(
      Layer.succeed(WebhookManagementService, management),
      IntegrationEventRecorderLive
    ),
