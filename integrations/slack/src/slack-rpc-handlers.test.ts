...
import { SlackManagementRpcHandlersEffect } from "./slack-rpc-handlers";
...
describe("SlackManagementRpcHandlers", () => {
  layer(TestLayer)("slack management rpc handlers", (it) => {
    it.effect(
      "denies connect start on the free plan before the service is called",
      () =>
        Effect.gen(function* () {
          const calls: string[] = [];
          const handlers = yield* SlackManagementRpcHandlersEffect.pipe(
            Effect.provideService(SlackManagementService, makeService(calls))
          );
          const organizationId = yield* seedWorkspace();

          const error = yield* Effect.flip(
            handlers
              .SlackConnectStart({ organizationId })
              .pipe(
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(started.authorizeUrl.hostname).toBe("slack.com");
