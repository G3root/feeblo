...
/** PostgreSQL persistence boundary for lease ownership; it never performs provider I/O. */
export const makeIntegrationDeliveryWorkerRepository = Effect.gen(function* () {
    ...
    ({ leaseDurationMs, leaseOwner, limit }) =>
      mapPersistenceError(
        ...
        Effect.gen(function* () {
          ...
          yield* recordIntegrationDeliveryBacklog(Number(backlog?.count ?? 0));
