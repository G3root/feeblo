...
- **Provider**: an external integration family. V1 implements the `webhook` and `slack` providers.
...
- **Binding**: a future durable local-to-remote entity relationship used by bidirectional sync. It is not implemented for V1; Slack identities currently resolve through the user table (email match or stable synthetic email).
