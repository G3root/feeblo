...
2. **Sync the dependency range.** If `@feeblo/sdk` changed, update `@feeblo/sdk-react`'s dependency on it in `packages/sdk-react/package.json` to the matching caret range (for example `^0.0.2`). Consumers resolve the bindings against the published core, so this range is the compatibility contract.
...
6. **Smoke-check the result.** After both jobs finish: `pnpm view @feeblo/sdk version`, `pnpm view @feeblo/sdk-react dependencies` (the dependency must be a real range like `^0.0.2`, never `workspace:*`), and ideally an install from a scratch project.

## Why the workflow publishes with pnpm

`sdk-react` declares its dependency as `"@feeblo/sdk": "workspace:*"`. That protocol is a pnpm workspace feature: `pnpm publish` rewrites it into a real semver range inside the published tarball. `npm publish` does not understand it and would ship an uninstallable range to the registry. The core SDK currently has no dependencies, so it would survive either client — both jobs still use `pnpm publish` for uniformity.
...
Versions are independent per package; there is no lockstep requirement. In practice most releases touch the core SDK alone, and the React bindings follow only when their adapter surface or dependency range changes. Breaking changes to the widget's public API should ship as a major bump of `@feeblo/sdk` plus a matching dependency-range update in `@feeblo/sdk-react`.
