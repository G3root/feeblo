...
The public board (`apps/public-feature-board`) is a React SPA rendered inside Astro as a `client:only` island (`apps/web/src/pages/s/[...subDomain].astro`). It owns its own Paraglide catalog because most of its UI is board copy, not shared component labels.

## Pattern

[Paraglide monorepo Pattern 1](https://paraglidejs.com/monorepo) with the "shared UI package owns reusable messages" variant:

- The board compiles its own project with the `baseLocale` fallback strategy: `apps/public-feature-board/project.inlang/` + `messages/{en,de}.json` → git-ignored `apps/public-feature-board/src/paraglide/`.
...
2. Add the English copy to `apps/public-feature-board/messages/en.json` and the German copy to `de.json` (du-form German, brand names stay hardcoded).
