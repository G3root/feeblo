...
- Shared catalog: `project.inlang/` + `messages/{en,de}.json` at the repo root.
...
2. Add the English copy to the root `messages/en.json` and the German copy to `messages/de.json` (du-form German, brand names stay hardcoded).
